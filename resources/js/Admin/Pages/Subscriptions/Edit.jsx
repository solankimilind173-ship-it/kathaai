...
export default function AdminSubscriptionsEdit({ plan, featureDefinitions = [], featureValues = {} }) {
    const fv = typeof featureValues === 'object' && featureValues !== null ? featureValues : {};
    const initialFeatures = featureDefinitions.reduce(
        (acc, d) => ({ ...acc, [d.key]: fv[d.key] ?? (d.type === 'boolean' ? '0' : '') }),
        {},
    );
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route('admin.subscriptions.update', plan));
    };
